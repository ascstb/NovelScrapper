...
(function ($) {
  var BootstrapWizard = function (element, options) {
    ...
    this.$body = $(
      '<div class="wizard-body">' +
        ...
        '<div class="btn-group">' +
        '<button class="btn btn-danger wizard-cancel" type="button">' +
        this.options.buttonText.cancel +
        "</button>" +
        "</div>" +
        '<button class="btn btn-success wizard-submit pull-right" type="button">' +
        this.options.buttonText.submit +
        "</button>" +
        '<button class="btn btn-primary wizard-next pull-right" type="button">' +
        this.options.buttonText.next +
        "</button>" +
        '<button class="btn btn-default wizard-back pull-right" type="button">' +
        this.options.buttonText.back +
        "</button>" +
        "</div>" +
        "</div>" +
        "</div>"
  ...
  BootstrapWizard.prototype = {
    ...
    _init: function () {
      ...
      this.$backButton = $wizardFooter
        .find(".wizard-back")
        .on("click", this.back);
      this.$nextButton = $wizardFooter
        .find(".wizard-next")
        .on("click", this.next);
      this.$submitButton = $wizardFooter
        .find(".wizard-submit")
      ...
      if (this.options.cancelButton) $cancelButton.show();
      ...
      else this.$nav.find(".progress").hide();

      var contentHeight =
        this.options.height - $wizardFooter.outerHeight() - 30;
      ...
      $navList.on("click.bw", "li", function (event) {
        ...
        $li = $(this);
        if (!$li.is(".visited") || $li.is(".disabled")) return;
    ...
    show: function ($li) {
      ...
      if (this.$nav.find(".wizard-nav-header").is(":visible")) {
        ...
        if (($parentNav = this.$current.parent("ul.collapse")).length) {
          navTitle =
            $parentNav.parent("li").children("a").text() + " > " + navTitle;
        }
    ...
    _resize: function () {
      ...
      if (this.$nav.find(".wizard-nav-header").is(":visible")) {
        ...
        this.$body.height(
          560 - this.$nav.find(".wizard-nav-header").outerHeight() + "px"
        );
        this.$body
          .find(".wizard-content")
          .height(
            this.$body.innerHeight() -
              this.$body.find(".wizard-footer").outerHeight() -
              30 +
              "px"
          );
        this.$nav.find(".wizard-nav").collapse("hide");
    ...
    submit: function () {
      ...
      var relatedPane = wizard.$body.find(
        wizard.$current.find("a").attr("href")
      );
